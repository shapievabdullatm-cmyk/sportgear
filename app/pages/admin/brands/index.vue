...
async function load(page = 1) {
  ...
  try {
    ...
    const res = await $api(`/admin/brands?${params}`)
...
<template>
  <div class="space-y-4 sm:space-y-5">
    ...
    <div class="flex items-center justify-between">
      <h1 class="text-[15px] font-semibold text-[#1A1A1A]">Бренды</h1>
      <NuxtLink
    ...
    <div v-else-if="!items.length" class="bg-white border border-[#E8E6E0] rounded-xl px-5 py-12 text-center text-[13px] text-[#C0BEB8]">
      {{ searchQuery ? 'Ничего не найдено' : 'Бренды не найдены' }}
    </div>
      ...
      <div class="sm:hidden space-y-2">
        ...
        >
          ...
          <div class="w-12 h-12 rounded-lg overflow-hidden bg-[#F0EEE9] flex items-center justify-center shrink-0">
            <img v-if="brand.image" :src="brand.image" :alt="brand.name" class="w-full h-full object-contain p-1.5" />
            <span v-else class="text-[9px] text-[#C0BEB8]">Нет</span>
          </div>
          <!-- Инфо -->
          <div class="flex-1 min-w-0">
            <div class="text-[13px] font-medium text-[#1A1A1A] truncate">{{ brand.name }}</div>
            <div class="text-[11px] text-[#ABABAB] font-mono truncate mt-0.5">{{ brand.slug }}</div>
          </div>
          <!-- Действия -->
          <div class="flex items-center gap-1 shrink-0">
            ...
            >
              <div v-if="deletingId === brand.id" class="w-3.5 h-3.5 border-2 border-[#E8E6E0] border-t-[#C0BEB8] rounded-full animate-spin" />
      ...
      <div class="hidden sm:block bg-white border border-[#E8E6E0] rounded-xl overflow-hidden">
        <div class="overflow-x-auto">
          <table class="w-full min-w-[560px] border-collapse">
            ...
            >
              <td class="td font-mono text-[11px] text-[#C0BEB8]">{{ brand.id }}</td>
              <td class="td">
                <div class="w-28 h-12 rounded-lg overflow-hidden bg-[#F0EEE9] flex items-center justify-center shrink-0">
                  <img v-if="brand.image" :src="brand.image" :alt="brand.name" class="w-full h-full object-contain p-2" />
                  <span v-else class="text-[10px] text-[#C0BEB8]">Нет</span>
                </div>
              </td>
              <td class="td text-[13px] font-medium text-[#1A1A1A]">{{ brand.name }}</td>
              <td class="td text-[13px] text-[#888] font-mono">{{ brand.slug }}</td>
              <td class="td">
                <div class="flex items-center justify-end gap-0.5">
                  ...
                  <button
                      ...
                      @click="deleteBrand(brand.id)"
                  >
                    <div v-if="deletingId === brand.id" class="w-3 h-3 border-2 border-[#E8E6E0] border-t-[#C0BEB8] rounded-full animate-spin" />
