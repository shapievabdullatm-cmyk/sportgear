...
import { useManufacturingCountryStore } from '~/stores/admin/manufacturing-country'

definePageMeta({ layout: 'admin', middleware: 'admin' })

const router = useRouter()
const route = useRoute()
const store = useManufacturingCountryStore()
...
onMounted(async () => {
  try {
    const { $api } = useApi()
    const data = await $api(`/admin/manufacturing-countries/${id.value}`)
    const country = data.data || data
    form.name = country.name
    form.slug = country.slug || ''
    existingFlagUrl.value = country.flag_url
    if (country.flag_url) {
      flagPreview.value = country.flag_url
    }
  } catch (e) {
    console.error(e)
  } finally {
...
async function save() {
  ...
  try {
    ...
    router.push('/admin/manufacturing-countries')
  } catch (e) {
  ...
  <div class="space-y-4 sm:space-y-5">
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-3">
        <NuxtLink
            to="/admin/manufacturing-countries"
        ...
        <h1 class="text-[15px] font-semibold text-[#1A1A1A]">Редактировать страну производства</h1>
      </div>
    </div>
