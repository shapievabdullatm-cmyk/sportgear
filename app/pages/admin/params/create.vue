<template>
  <div class="max-w-2xl mx-auto space-y-5">
    ...
    <div class="flex items-center justify-between">
      <h1 class="text-[15px] font-semibold text-[#1A1A1A]">Новый атрибут</h1>
      <NuxtLink to="/admin/params"
                class="inline-flex items-center gap-1.5 text-[13px] font-medium text-[#ABABAB] hover:text-[#1A1A1A] transition-colors">
    ...
    <div class="bg-white border border-[#E8E6E0] rounded-xl p-6 space-y-4">
      ...
      <div class="space-y-1.5">
        ...
        <input v-model="form.title" @input="onTitleInput" type="text" placeholder="Например: Цвет, Материал, Вес"
      ...
      <div class="space-y-1.5">
        ...
        <div class="relative flex items-center">
          <input v-model="form.slug" @input="onSlugInput" type="text" placeholder="auto-generated"
                 class="w-full px-3.5 py-2.5 pr-10 rounded-lg border text-[13px] text-[#1A1A1A] font-mono placeholder-[#C0BEB8] outline-none transition-colors"
        ...
        <p v-else class="text-[11px] text-[#C0BEB8]">
          /catalog?<span class="text-[#888]">{{ form.slug || 'slug' }}</span>=value
          <span v-if="slugChecking" class="ml-1.5 text-[#ABABAB]">· проверка…</span>
          <span v-else-if="slugManual" class="ml-1.5 text-[#ABABAB]">· ручной режим</span>
        </p>
      </div>
      ...
      <div v-if="form.filter_type !== null" class="space-y-1.5">
        ...
        <div class="flex items-center gap-2">
          <input v-model="form.unit" type="text" placeholder="г, кг, см…"
                 class="w-48 px-3.5 py-2.5 rounded-lg border border-[#E8E6E0] bg-[#FAFAF8] text-[13px] text-[#1A1A1A] placeholder-[#C0BEB8] outline-none focus:border-[#1A1A1A] focus:bg-white transition-colors" />
    ...
    <div class="bg-white border border-[#E8E6E0] rounded-xl p-6 space-y-4">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <ToggleSetting v-model="form.is_filterable" label="В фильтрах" description="Показывать в боковых фильтрах каталога" />
        <ToggleSetting v-model="form.is_searchable" label="В поиске" description="Учитывать при полнотекстовом поиске" />
        <ToggleSetting v-model="form.is_comparable" label="В сравнении" description="Показывать при сравнении товаров" />
      </div>
      <div class="flex items-center gap-3 pt-1">
        ...
        <p class="text-[11px] text-[#ABABAB]">Меньше = выше в списке атрибутов</p>
      </div>
    </div>
      ...
      <div v-if="isColorType" class="bg-white border border-[#E8E6E0] rounded-xl p-6 space-y-3">
        ...
        <div v-for="(opt, idx) in form.colorOptions" :key="idx"
          ...
          <div class="flex items-center gap-2.5">
            ...
            <input v-model="opt.name" @input="onColorNameInput(opt)" type="text" placeholder="Название цвета (Красный, Закат…)"
    ...
    <Transition name="slide">
      <div v-if="isBooleanType" class="bg-white border border-[#E8E6E0] rounded-xl p-5 flex items-start gap-3">
        <div class="w-8 h-8 rounded-lg bg-[#F0EEE9] flex items-center justify-center shrink-0 mt-0.5">
        ...
        <div>
          ...
          <p class="text-[12px] text-[#888] mt-0.5 leading-relaxed">Вариантов не нужно — покупатель увидит переключатель <strong>Да / Нет</strong>.<br>Примеры: оригинальный товар, водонепроницаемый, беспроводной.</p>
        </div>
      </div>
    </Transition>
    ...
    <div class="flex justify-end">
      <button type="button" :disabled="!isFormValid" @click="submit"
...
import type { FilterTypeItem } from '~/types/param'
import { ParamFilterType, slugify, serializeColorValue, type ColorKind } from '~/types/param'
...
onMounted(() => {
  const handler = (e: KeyboardEvent) => {
    if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') { e.preventDefault(); if (isFormValid.value) submit() }
  }
...
const { data: pageData } = await useAsyncData('param-create', async () => {
  const { $api } = useApi()
  return $api<{ filterTypes: FilterTypeItem[] }>('/admin/params/create')
})
...
interface ColorOpt  { name: string; slug: string; slugManual: boolean; type: ColorKind; stops: ColorStop[]; angle: number }
...
// ── Form ──────────────────────────────────────────────────────────────────────
interface ListOption { value: string; slug: string; slugManual: boolean }

const form = reactive({
  title: '', slug: '', filter_type: null as number | null, unit: '',
  is_filterable: true, is_searchable: false, is_comparable: false, sort: 0,
  options:      [{ value: '', slug: '', slugManual: false }] as ListOption[],
  colorOptions: [defaultColor()] as ColorOpt[],
})
...
async function checkSlugAvailability(slug: string) {
  ...
  try {
    ...
    const res = await $api<{ slug: string; available: boolean }>(
        `/admin/params/check-slug?slug=${encodeURIComponent(slug)}`
    )
    if (!res.available) {
...
function onOptionValueInput(opt: ListOption) {
...
async function submit() {
  ...
  const options = isListType.value
      ? form.options.filter(o => o.value.trim()).map(o => ({ value: o.value.trim(), slug: o.slug || slugify(o.value) }))
      : isColorType.value
          ? form.colorOptions.map(o => ({ value: serializeOpt(o), slug: o.slug || slugify(o.name) }))
          : []
  try {
    await store.store({
  ...
  showToast('Атрибут создан', form.title.trim())
  form.title = ''; form.slug = ''; form.filter_type = null; form.unit = ''
  form.is_filterable = true; form.is_searchable = false; form.is_comparable = false; form.sort = 0
  form.options = [{ value: '', slug: '', slugManual: false }]
  form.colorOptions = [defaultColor()]
  slugManual.value = false
}
</script>
...
export const ToggleSetting = defineComponent({
  name: 'ToggleSetting',
  props: { modelValue: Boolean, label: String, description: String },
  emits: ['update:modelValue'],
  setup(props, { emit }) {
    return () => h('div',
        { class: 'flex items-start gap-3 p-3 rounded-lg border border-[#E8E6E0] bg-[#FAFAF8] cursor-pointer select-none', onClick: () => emit('update:modelValue', !props.modelValue) },
        [
          h('div', { class: 'relative mt-0.5 shrink-0' }, [
            h('div', { class: `w-9 h-5 rounded-full transition-colors duration-200 ${props.modelValue ? 'bg-[#1A1A1A]' : 'bg-[#D1CEC9]'}` }),
            h('div', { class: `absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow-sm transition-transform duration-200 ${props.modelValue ? 'translate-x-4' : 'translate-x-0'}` }),
          ]),
          h('div', { class: 'flex-1 min-w-0' }, [
            h('p', { class: 'text-[12px] font-medium text-[#1A1A1A]' }, props.label),
            h('p', { class: 'text-[11px] text-[#ABABAB] mt-0.5 leading-relaxed' }, props.description),
          ]),
        ]
    )
  },
