      ...
      <div v-if="toast.visible.value" class="fixed top-5 right-5 z-[60] w-72">
        <div class="bg-white border border-[#E8E6E0] rounded-xl shadow-lg overflow-hidden">
          <div class="p-4 flex items-center gap-3">
            ...
            <p class="text-[13px] font-semibold text-[#1A1A1A] flex-1">Товар создан</p>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div class="lg:col-span-2 space-y-5 min-w-0">
          ...
          <section class="bg-white border border-[#E8E6E0] rounded-xl p-5 sm:p-6">
            ...
            <ImageManager
                :images="[
                  ...pendingPreviews.map((preview, i) => ({ preview, id: undefined, url: preview })),
                  ...copiedImagePreviews.map((url, i) => ({ preview: url, id: undefined, url: url, isCopied: true }))
                ]"
        ...
        >
          ...
          <!-- Input — используем pv() чтобы гарантировать инициализацию -->
          ...
          <!-- Barcodes -->
          <ProductBarcodes v-model="barcodes" />
            ...
            <section class="bg-white border border-[#E8E6E0] rounded-xl p-5">
              ...
              <label class="flex items-center justify-between cursor-pointer group">
                <div>
                  ...
                  <p class="text-[11px] text-[#888] mt-0.5">{{ form.is_active ? 'Товар будет виден в каталоге' : 'Скрыт от покупателей' }}</p>
                </div>
                <div class="relative">
...
import ProductBarcodes from '~/components/admin/ProductBarcodes.vue'
import ProductCopyModal from '~/components/admin/ProductCopyModal.vue'

definePageMeta({ layout: 'admin', fullBleed: true })
...
// ── Init data ─────────────────────────────────────────────────────────────────
const { data: initData } = await useAsyncData('product-create', () =>
    $api<{ categories: Category[]; brands: any[]; brandOrigins: any[]; manufacturingCountries: any[]; products: Product[] }>('/admin/products/create')
)
...
// Parent products - только товары без родителя
const parentProductOptions = computed(() => [
    { value: null, label: 'Не выбрано' },
    ...products.value
        .filter(p => !p.parent_id) // Только товары без родителя
...
const defaultForm = () => ({
  ...
  brand_origin_id:          null as number | null,
  manufacturing_country_id: null as number | null,
  size_table_id:            null as number | null,
...
// ── Barcodes ──────────────────────────────────────────────────────────────────
const barcodes = ref<Array<{ barcode: string; type: string | null }>>([])
...
function onImagesReorder(newImages: any[]) {
  // Reorder files, previews, and copied images based on new order
  const newFiles: File[] = []
  ...
  newImages.forEach(img => {
    if (img.isCopied) {
      ...
      if (oldIndex >= 0) {
        newFiles.push(pendingFiles.value[oldIndex])
        newPreviews.push(pendingPreviews.value[oldIndex])
      }
    }
  })

  pendingFiles.value = newFiles
  pendingPreviews.value = newPreviews
...
async function handleCopyData(sourceProduct: Product, options: Record<string, boolean>) {
  try {
    ...
    if (options.category && product.category_id) {
      ...
      await loadParams(product.category_id)
    }
    ...
    if (options.params && resolvedParams.value.length > 0 && product.raw_params) {
      // Здесь параметры уже должны быть загружены через loadParams
      // Просто копируем значения из исходного товара
...
async function submit() {
  ...
  try {
    ...
    barcodes.value.forEach((barcode, index) => {
      fd.append(`barcodes[${index}][barcode]`, barcode.barcode)
      if (barcode.type) {
        fd.append(`barcodes[${index}][type]`, barcode.type)
      }
    })
