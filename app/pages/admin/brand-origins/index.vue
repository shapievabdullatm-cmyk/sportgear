...
async function load(page = 1) {
  ...
  try {
    ...
    const res = await $api(`/admin/brand-origins?${params}`)
...
async function deleteOrigin(id) {
  if (!confirm('Удалить родину бренда?')) return
  deletingId.value = id
  try {
...
<template>
  <div class="space-y-4 sm:space-y-5">
    ...
    <div class="flex items-center justify-between">
      <h1 class="text-[15px] font-semibold text-[#1A1A1A]">Родина бренда</h1>
      <NuxtLink
          to="/admin/brand-origins/create"
    ...
    <div v-else-if="!items.length" class="bg-white border border-[#E8E6E0] rounded-xl px-5 py-12 text-center text-[13px] text-[#C0BEB8]">
      {{ searchQuery ? 'Ничего не найдено' : 'Родины брендов не найдены' }}
    </div>
      ...
      <div class="sm:hidden space-y-2">
        ...
        >
          ...
          <div class="w-12 h-12 rounded-lg overflow-hidden bg-[#F0EEE9] flex items-center justify-center shrink-0">
            <img v-if="origin.flag_url" :src="origin.flag_url" :alt="origin.name" class="w-full h-full object-contain p-1.5" />
            <span v-else class="text-[9px] text-[#C0BEB8]">Нет</span>
          </div>
          <!-- Инфо -->
          <div class="flex-1 min-w-0">
            <div class="text-[13px] font-medium text-[#1A1A1A] truncate">{{ origin.name }}</div>
            <div class="text-[11px] text-[#ABABAB] font-mono truncate mt-0.5">{{ origin.slug }}</div>
          </div>
          <!-- Действия -->
          <div class="flex items-center gap-1 shrink-0">
            <NuxtLink
                :to="`/admin/brand-origins/${origin.id}/edit`"
            ...
            <button
                ...
                @click="deleteOrigin(origin.id)"
            >
              <div v-if="deletingId === origin.id" class="w-3.5 h-3.5 border-2 border-[#E8E6E0] border-t-[#C0BEB8] rounded-full animate-spin" />
      ...
      <div class="hidden sm:block bg-white border border-[#E8E6E0] rounded-xl overflow-hidden">
        <div class="overflow-x-auto">
          <table class="w-full min-w-[560px] border-collapse">
            ...
            >
              <td class="td font-mono text-[11px] text-[#C0BEB8]">{{ origin.id }}</td>
              <td class="td">
                <div class="w-28 h-12 rounded-lg overflow-hidden bg-[#F0EEE9] flex items-center justify-center shrink-0">
                  <img v-if="origin.flag_url" :src="origin.flag_url" :alt="origin.name" class="w-full h-full object-contain p-2" />
                  <span v-else class="text-[10px] text-[#C0BEB8]">Нет</span>
                </div>
              </td>
              <td class="td text-[13px] font-medium text-[#1A1A1A]">{{ origin.name }}</td>
              <td class="td text-[13px] text-[#888] font-mono">{{ origin.slug }}</td>
              <td class="td">
                <div class="flex items-center justify-end gap-0.5">
                  ...
                  <button
                      ...
                      @click="deleteOrigin(origin.id)"
                  >
                    <div v-if="deletingId === origin.id" class="w-3 h-3 border-2 border-[#E8E6E0] border-t-[#C0BEB8] rounded-full animate-spin" />
